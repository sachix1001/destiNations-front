...
public class WatsonTextToSpeech : MonoBehaviour
{
    ...
    public static string greeting = "Hello! In the future I will be able to speak other languages too!";
    public VoiceType currentVoice = VoiceType.en_US_Allison;

    TextToSpeech _service;
    string _testString = "<speak version=\"1.0\"><prosody pitch=\"150Hz\">" + greeting + "</prosody></speak>";
    string _helloString = "<speak version=\"1.0\"><say-as interpret-as=\"letters\">I'm sorry</say-as>. <prosody pitch=\"150Hz\">This is Text to Speech!</prosody><express-as type=\"GoodNews\">I'm sorry. This is Text to Speech!</express-as></speak>";
    ...
    void Start()
    {
        ...
        GameObject UI = GameObject.Find("/UIComponents");
        var bubble = UI.transform.GetChild(7).GetChild(3);
        var hello = bubble.GetChild(1);

        bubble.GetComponent<Button>().onClick.AddListener(() =>
        {
            LogSystem.InstallDefaultReactors();
            Runnable.Run(CreateService());
        });

        hello.GetComponent<Button>().onClick.AddListener(() =>
        {
            LogSystem.InstallDefaultReactors();
            Runnable.Run(CreateService());
        });
    ...
    private IEnumerator Examples()
    {
        ...
        //	Get Voice
        Log.Debug("ExampleTextToSpeech.Examples()", "Attempting to get voice {0}.", VoiceType.ja_JP_Emi);
        _service.GetVoice(OnGetVoice, OnFail, VoiceType.ja_JP_Emi);
        while (!_getVoiceTested)
            yield return null;

        //	Get Pronunciation
        Log.Debug("ExampleTextToSpeech.Examples()", "Attempting to get pronunciation of {0}", _testWord);
        _service.GetPronunciation(OnGetPronunciation, OnFail, _testWord, VoiceType.ja_JP_Emi);
